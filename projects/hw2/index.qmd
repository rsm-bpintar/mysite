...
The coefficient on iscustomer is 0.208, meaning that, holding all else equal, firms that use Blueprinty's software have a 23% higher expected patent count compared to non-customers. See. 
...
We'll now try to quantify the effect of using Blueprinty's fotware by simulating predicted patent counts for the firms in our data in two scenaqrios, one where all firms are non-customers, and one where all were customers, holding all else constant. 
...
We find that the average predicted number of patents increased by about 0.973 when firms used Blueprinty's software. Resultingly, Blueprinty customers are expected to recieve one additional patent over a five year period compared to similar non-customers.
